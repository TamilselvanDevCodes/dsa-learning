...
public class BuyAndSellStocksWithCooldown {
    ...
    public static void main(String[] args) {
        BuyAndSellStocksWithCooldown main = new BuyAndSellStocksWithCooldown();
...
//        return getMaxProfitWithDP(prices);
    ...
    private int getMaxProfitWithDP(int index, int canBuy,int[][]dp, int[] prices) {
        ...
        if (canBuy == 1) {
            ...
            profit = Math.max(getMaxProfitWithDP(index + 2, 1,dp, prices) + prices[index],
                    getMaxProfitWithDP(index + 1, canBuy, dp,prices)
            );
        }
    ...
    private int getMaxProfit(int index, int canBuy, int[] prices) {
        ...
        if (canBuy == 1) {
            ...
            profit = Math.max(getMaxProfit(index + 2, 1, prices) + prices[index],
                    getMaxProfit(index + 1, canBuy, prices)
            );
        }
        return profit;
